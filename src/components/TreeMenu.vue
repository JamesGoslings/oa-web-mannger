<template>
  <div class="treeAll" v-for="(menu, i) in menuList" :key="i">
    <div v-if="!menu.hidden">
      <div
        class="menuBar"
        :style="{ background: backColors[i] }"
        @click="routerPash(menu)"
      >
        <div
          class="menuOne"
          :style="{
            paddingLeft: floor + 'vw',
            background: backColors[i],
            color: clickOtherColors[i],
          }"
          @click="changeStyle(i, menu, menuList)"
          @mouseenter="backColors[i] = actStyle.backColor"
          @mouseleave="resetColor(i, menu)"
        >
          <div class="msg">
          ...
          <span v-if="menu.alwaysShow">
            <div
              class="iconfont chooseIco"
              :style="{ marginRight: floor + 'vw' }"
              v-if="!isChoose[i]"
            >
              &#xe600;
            </div>
