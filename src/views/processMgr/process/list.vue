      ...
      <div class="table">
        <el-table :data="pageData" style="width: 100%">
          ...
          <el-table-column label="当前申请状态" width="150">
            <template v-slot="scope">
              <div
                class="statusShow"
                :style="{
                  background: getStatusObj(scope.row.status).backColor,
                  color: getStatusObj(scope.row.status).fontColor,
                }"
              >
                {{ getStatusObj(scope.row.status).name }}
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column fixed="right" label="操作" width="250">
            <template v-slot="scope">
              <div class="tabFooter">
                <el-button size="small" plain @click="showFormData(scope.row)"
                  >查看申请表单</el-button
                >
              </div>
            </template>
...
const selectPageBykey = async () => {
  ...
  let { data } = await listPageProcess(
    page.value,
    limit.value,
    searchObj.value
  );
...
function getActStyle(i) {
  ...
  return chooseIndex.value === i
    ? { color: actColor, borderBottom: `${actColor} 2.5px solid` }
    : "";
}
onMounted(() => {
  listProcessType();
