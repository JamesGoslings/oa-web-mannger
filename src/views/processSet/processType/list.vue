    ...
    <div class="tableBox baseCard">
      ...
      >
        ...
        <el-radio-group v-model="chooseProcessTypeIndex" v-if="funType === 2">
          <div
            v-for="(processType, i) in allProcessTypes"
            :key="processType.id"
            class="choiceRow"
        ...
        <el-checkbox-group v-model="chooseTypes" v-else>
          <div
            class="choiceRow"
            v-for="processType in allProcessTypes"
            :key="processType.id"
            :title="getTypeMsgStr(processType)"
          >
        ...
        <template #footer>
          <div class="dialog-footer">
            ...
            <el-button
              ...
              :disabled="chooseProcessTypeIndex < 0 && funType === 2"
              >确定</el-button
            >
          </div>
        </template>
      ...
      >
        <el-form :model="form">
          ...
          <el-form-item label="类型描述">
            <el-input
              v-model="checkedProcessType.description"
              autocomplete="off"
            />
          </el-form-item>
        </el-form>
...
import {
  getAllProcessTypes,
  getProcessTypePage,
  removeOneProcessType,
  saveProcessType,
  updateProcessType,
  batchRemoveProcessTypes,
} from "@/api/processType";
...
function editThisType() {
  useSimpleConfirm(
    `你确定要保存对类型：“${checkedProcessType.value.name}”的修改吗？？？`
  ).then(async () => {
...
function removeOne(typeOne) {
  useSimpleConfirm(`你真的要删除审批类型：“${typeOne.name}” 吗？？？`).then(
    async () => {
...
const getPageData = async () => {
  ...
  let { data } = await getProcessTypePage(
    page.value,
    limit.value,
    queryObj.value
  );
