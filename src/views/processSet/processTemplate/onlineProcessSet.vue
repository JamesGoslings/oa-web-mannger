...
function saveAndQuit() {
  useSimpleConfirm(
    `你确定要保存 “${thisTemplate.value.name}” 模板的流程图吗？`
  ).then(async () => {
    // await updateFormMsg()
    exportXML();
    // router.go(-1)
...
function exportXML() {
  bpmnModeler.value.saveXML({ format: true }, function (err, xml) {
    if (err) {
      ...
      let myXml = xml
        .replace(/camunda.org\/schema\/1.0\/bpmn/gi, "activiti.org/bpmn")
        .replace(/camunda/gi, "activiti");
      console.log("导出的XML:");
      console.log(myXml);
      saveImgMsg(myXml);
    }
...
const getThisXmlStr = async () => {
  ...
  let camundaXml = data.xmlStr
    .replace(/activiti.org\/bpmn/gi, "camunda.org/schema/1.0/bpmn") // 替换Activiti的命名空间为Camunda的
    .replace(/activiti/gi, "camunda"); // 替换所有Activiti文本为Camunda
  // 导入xml字符串
  if (camundaXml) {
