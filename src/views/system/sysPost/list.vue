  ...
  <div class="postAll">
          ...
          <div class="formFun">
            <el-form :model="form">
              <div v-for="(item, i) in formMsgField" :key="i">
                <el-form-item
                  :label="item.label"
                  v-if="funType === 1 || (funType === 0 && item.type !== 2)"
                >
                  <el-input
                    v-model="checkedPost[item.field]"
                    autocomplete="off"
              ...
              <div class="footer">
                <el-button
                  v-if="funType === 1"
                  :disabled="!checkedPost.id"
                  @click="removeThisPost()"
                  type="danger"
                  >删除</el-button
                >
                <el-button
                  type="primary"
                  @click="saveOrUpdateOPost()"
                  :disabled="
                    (!checkedPost.id && funType === 1) ||
                    (funType === 0 &&
                      !(checkedPost.name && checkedPost.postCode))
                  "
                  >保存</el-button
                >
              </div>
    ...
    <el-dialog v-model="openDialog" title="全部岗位" width="900" append-to-body>
      ...
      <el-radio-group v-model="choosePostIndex">
        <el-radio
          :value="post.id"
          v-for="post in totalPosts"
          :key="post.id"
          @click="choosecard(post)"
        >
          <div class="myCard">
            <div class="msg">
    ...
    >
      ...
      <el-input
        ...
        @input="checkIptNum()"
        v-model="iptKeyword"
        :placeholder="`请输入显示数目 0~${totalPosts.length}`"
        autocomplete="off"
      />
      <el-button type="primary" @click="setShowNum()" :disabled="!checkIptNum()"
        >确定</el-button
      >
    </el-dialog>
  </div>
...
const getPostsByKey = async () => {
  let { data } = await getTotalPostsByKeyword(
...
function removeThisPost() {
  useSimpleConfirm(
    `你确定要删除岗位 “${checkedPost.value.name}” 吗？？？`
  ).then(async () => {
...
function updateThisPost() {
  useSimpleConfirm(`你确定保存对 “${checkedPost.value.name}” 的修改吗？`).then(
    async () => {
...
const changePostCode = async () => {
  ...
  let {
    data: { newPostCode },
  } = await getNewCode(post.id, post.deptId, post.type);
  checkedPost.value.postCode = newPostCode;
};
// 设置提示框信息
function getTipContent() {
  const SaveTip =
    "岗位编码在选择部门和类型之后会自动生成，推荐使用自动生成的编码";
