<template>
  <div class="menuPageAll">
    ...
    <div class="funs">
      <div class="funBar">
        <myInputBar
          back-color="rgb(229,231,235)"
          font-color="rgb(36,47,87)"
          radius="35px"
          holder-color="rgb(183,190,200)"
          style="height: 4vh; width: 55vw"
    ...
    >
      <el-form :model="form">
        ...
        <el-form-item
          v-if="checkedMenu.parentName != null && checkedMenu.parentName != ''"
        >
          <div></div>
        </el-form-item>
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button v-if="funType === 1" type="danger" @click="removeOneMenu()"
            >删除</el-button
          >
          <el-button type="primary" @click="saveOrupdateChangeMenu()"
            >保存</el-button
          >
        </div>
      </template>
...
function updateChangeMenu() {
  ...
  useSimpleConfirm(
    `你确定要保存对菜单 “${checkedMenu.value.totalName}” 的修改吗?`
  ).then(async () => {
    // 防止修改日期不同步的问题
...
function removeOneMenu() {
  useSimpleConfirm(
    `你确定要删除菜单 “${checkedMenu.value.totalName}” 吗？？？`
  ).then(async () => {
