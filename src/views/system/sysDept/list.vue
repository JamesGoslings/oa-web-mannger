  ...
  <div class="deptAll">
    <div class="funTitle myTitle">
      ...
      <changeSwitch
        ...
        radius="10px"
        border-size="10px"
        back-color="rgb(244,247,252)"
        act-back-color="rgb(254,255,255)"
        act-font-color="rgb(36,47,87)"
        left-txt="部门信息"
        right-txt="统计信息"
        ...
        <div class="msgAll baseCard">
          <div class="deptMsg">
            ...
            <div v-for="(item, i) in deptMsgFields" :key="i">
              <div class="msgOne" v-if="item.field !== '__none'">
                {{ `${item.label}：${checkedDept[item.field]}` }}
              </div>
              <div class="msgOne" v-else-if="item.type === 1">
                {{ getCountStr(item) }}
              </div>
              <div class="msgOne" v-else>
                {{ `${item.label}：${checkedDept.leader.name}` }}
              </div>
            </div>
          </div>
          <div class="userMsg">
            ...
            <div v-for="(item, i) in userMsgFields" :key="i">
              <div class="msgOne">
                {{ `${item.label}：${checkedDept.leader[item.field]}` }}
              </div>
            </div>
          </div>
        </div>
            ...
            <el-form :model="form">
              <div v-for="(item, i) in formMsgField" :key="i">
                <el-form-item :label="item.label">
                  ...
                  <el-radio-group
                    v-model="editDept.isAddChildrenCount"
              ...
              <div class="footer">
                <el-button
                  ...
                  type="danger"
                  @click="removeThisDept()"
                  >删除</el-button
                >
                <el-button
                  type="primary"
                  :disabled="isDisAbled()"
                  @click="saveOrUpDateDept()"
                  >保存</el-button
                >
              </div>
            </el-form>
    ...
    <div class="baseCard treeImg">
      <div style="margin-right: 10px">
        <el-switch v-model="horizontal"></el-switch> 横向
      </div>
      <vue3-tree-org
        :data="treeData"
      ...
      >
        ...
        <template v-slot="{ node }">
          <div class="nodeOne">
            ...
            <div v-if="node.$$data.id !== 0">
              部门人数：{{ node.$$data.totalCount }}
            </div>
    ...
    >
      ...
      <div class="iptAll">
        ...
        <div class="iptMsg">
          ...
          <el-input
            ...
            @input="checkIptNum()"
            v-model="settingMsg.showNum"
            :placeholder="`请输入显示部门数目 0~${totalDepts.length}`"
            autocomplete="off"
          />
        </div>
      </div>
      <div class="btnAll">
        <el-button
          type="primary"
          @click="applySettingMsg()"
          :disabled="!checkIptNum()"
          >确定</el-button
        >
...
function handleEnter() {
  if (isSpace(iptKeyword.value)) {
    ...
    totalDepts.value = filterSimilarObjects(
      totalDepts.value,
      removeWhiteSpaces(iptKeyword.value)
    );
  }
}
// 检索的方法
function filterSimilarObjects(objects, str) {
  ...
  return objects.filter((obj) => {
    return (
      (obj.name && obj.name.includes(str)) ||
      (obj.deptCode && obj.deptCode.includes(str))
    );
  });
}
// 用于存输入的关键字
...
function removeThisDept() {
  useSimpleConfirm(`你真的确定要删除部门：${editDept.value.name} 吗？`).then(
    async () => {
...
function isDisAbled() {
  ...
  return (
    (!myDept.id && funType.value === 1) ||
    (funType.value === 0 &&
      !(
        myDept.parentId != null &&
        myDept.name &&
        myDept.deptCode &&
        myDept.leaderId &&
        myDept.isAddChildrenCount
      ))
  );
}
// 动态获取新的编码
const getThisNewDeptCode = async () => {
  ...
  let {
    data: { newDeptCode },
  } = await getNewDeptCode(deptId, parentId);
...
function updateThisDept() {
  useSimpleConfirm(`你确定要修改部门：${editDept.value.name} 吗？？？`).then(
    async () => {
...
function getTipContent() {
  return funType.value === 1
    ? "1)部门编码会随着上级部门的修改而自动改变，建议使用自动生成的编码; 2)计算属性指的是在计算部门人数的时候，是否要算上其子部门人数"
...
function setEchartsData() {
  ...
  for (let j = 0; j < totalDepts.value.length; j++) {
    if (totalDepts.value[j].parentId === settingMsg.value.rootDeptId) {
      ...
      deptTotalCountWithChildrernData.value[x].value =
        totalDepts.value[j].totalCount;
