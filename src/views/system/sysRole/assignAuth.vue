...
import {getMenuListByRoleId,getAllTreeMenus,getMenuIdListWithoutChilrenByRoleId,doAssignForRole} from '@/api/menu'


// 保存并退出
function saveAndQuit (){
    useSimpleConfirm(`你确定要保存对角色 “${role.value.roleName}” 的权限修改吗`).then(async ()=>{
        let data = await doAssignForRole({roleId: role.value.id,menuIdList: chooseIdList.value});
