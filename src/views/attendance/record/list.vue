    ...
    <div class="funAll">
      ...
      <el-card class="statistic" shadow="hover">
        <div class="myBar">
          ...
          <div>
            <el-button style="margin-right: 1vw" @click="openDialog = true"
              >选择部门</el-button
            >
            <SelectDaysBar
              @change-days="getDeptDays"
              bar-width="7vw"
              :chooseDaysStr="[1, 7, 15, 30]"
            />
          </div>
        </div>
    ...
    >
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button
            type="primary"
            @click="getDeptsRadius()"
            :disabled="chooseDeptIds.length === 0"
            >确定</el-button
          >
        </div>
      </template>
...
const getNotUsers = async () => {
  let { data } = await listNotUsersInDept(
    chooseDeptId.value,
    chooseType.value,
    chooseUserDay.value
  );
...
const getDeptsRadius = async () => {
  let { data } = await listDeptRadius(
    chooseDeptIds.value,
    chooseDeptDay.value - 1
  );
